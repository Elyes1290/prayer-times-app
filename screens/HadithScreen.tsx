...
export default function HadithScreen() {
  ...
  const convertToFavorite = (
    ...
    return {
      ...
      translation: item.hadithEnglish, // ✅ Correction: utilisé le bon nom de champ
  ...
  useEffect(() => {
    ...
    // 🚀 DEBUG: Afficher l'API key utilisée
    ...
    fetch(`https://hadithapi.com/api/books?apiKey=${API_KEY}`)
      .then((res) => {
        //console.log("📚 Réponse API books - Status:", res.status);
        return res.json();
      })
      .then((data) => {
        //console.log("📚 Data books reçue:", data);
        setBooks(data.books || []);
        setLoadingBooks(false);
      })
      .catch((error) => {
        console.error("❌ Erreur fetch books:", error);
        setLoadingBooks(false);
      });
  }, []);
